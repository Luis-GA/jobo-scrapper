...
from tests import JOBO_WEBPAGE_WITHOUT_EVENTS, JOBO_LOGIN, JOBO_WEBPAGE_WITH_EVENT, JOBO_WEBPAGE_BROKEN
...
def test_return_available_event(mock_session_get, mock_session_post):
    ...
    assert event.get("title") and event.get("image") and event.get("place") and event.get("link") and event.get("days") and event.get("description")
